        ...
        request.enqueue(object : Callback<ResponseBody> {
            ...
            private fun getCheckExistResponse(res: String) {
                ...
                if (status.equals(Constants.Status.SUCCESS.name, true)) {
                    ...
                    observerResponse.value = CheckExistPhoneResponse(CheckExistPhoneResponse.Data(email, mobNo, userId,walletUUID,tpin, securityQuestion, userName,avatarUrl), errorCode, status)
                } else {
                    val message = json.getString("message")
                    observerResponse.value = CheckExistPhoneResponse(null, errorCode, status, message)
                }
            }
        })
    }
        ...
        request.enqueue(object : Callback<ResponseBody> {
            ...
            private fun getCheckExistFullResponse(res:String){
                ...
                if (status.equals(Constants.Status.SUCCESS.name, true)) {
                    ...
                    observerResponse.value = CheckUserFullResponse(status, errorCode, Constants.errorMessage(errorCode),null)
                }
            }
        })
    }
