...
class StackLayoutManager(
    ...
    visibleCount: Int,
    animation: Class<DefaultAnimation>,
    ...
    fun setPagerFlingVelocity( velocity: Int) {
        mPagerFlingVelocity = Int.MAX_VALUE.coerceAtMost(Math.max(0, velocity))
    }
    ...
    init {
        ...
        if (StackLayout::class.java.isAssignableFrom(layout)) {
            try {
                ...
                mLayout = cla.newInstance(scrollOrientation, visibleCount, 60) as StackLayout
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
    ...
    private fun handleScrollBy(offset: Int, recycler: RecyclerView.Recycler): Int {
        ...
        //The actual offset, the offset after exceeding the maximum and minimum values should be 0,
        // this value is used as the return value,
        // otherwise the elastic shadow will not appear when scrolling at the limit position
        val exactMove = mScrollOffset - expectOffset + offset

        if (exactMove == 0) {
            //The position of itemViews will not change, just return
            return 0
        }
