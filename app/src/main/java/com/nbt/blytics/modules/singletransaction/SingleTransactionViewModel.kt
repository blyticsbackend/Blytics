...
class SingleTransactionViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun transactionHistory(transactionRequest: TransactionRequest) {
        type =transactionRequest.bank
        val request: Call<ResponseBody> = service.transactionHistory(transactionRequest)
        request.enqueue(object : Callback<ResponseBody> {
            ...
            fun getTransactionResponse(res: String) {
                ...
                var total_charges =""
               // if(type =="Internal") {
                     amount_sent = data.getString("amount_sent")
                     amount_receive = data.getString("amount_receive")
                     total_charges = data.getString("total_charges")
               // }
